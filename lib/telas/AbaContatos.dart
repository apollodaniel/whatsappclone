    ...
    firestore.collection("users").snapshots().listen(
        (event){
          ...
          for(DocumentSnapshot doc in event.docs){
            ...
            Pessoa pessoa = Pessoa(nome: nome, email: doc.get("email"), profile_picture: doc.get("profile_picture"));
          ...
          return Card(
            child: ListTile(
              leading: GestureDetector(
                child: CircleAvatar(
                  backgroundImage: NetworkImage(pessoas[index].profile_picture),
                ),
                onTap: (){
                  showDialog(context: context, builder: (context) {
                    return AlertDialog(
                      title: Text(pessoas[index].nome),
                      content: Image.network(pessoas[index].profile_picture,height: 180, width: 180, fit: BoxFit.cover,),
